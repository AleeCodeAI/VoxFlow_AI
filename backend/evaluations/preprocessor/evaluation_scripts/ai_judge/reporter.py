...
class Reporter:
    ...
    def generate_summary(self, results: List[Result]):
        try:
            ...
            meaning_score = (
                sum(
                    metric_weights["meaning_preservation"][r.meaning_preservation]
                    for r in results
                )
                / (total * 3)
                if total > 0
            ...
            info_loss_score = (
                sum(
                    metric_weights["information_loss"][r.information_loss]
                    for r in results
                )
                / (total * 3)
                if total > 0
            ...
            quality_score = (
                sum(
                    metric_weights["preprocessing_quality"][r.preprocessing_quality]
                    for r in results
                )
                / (total * 3)
                if total > 0
            ...
            hallucination_score = (
                sum(metric_weights["hallucination"][r.hallucination] for r in results)
                / (total * 3)
                if total > 0
                else 0
            )
