...
from ai_judge import EvaluationClient
from ai_judge import PromptBuilder
from ai_judge import Storage
from ai_judge import Reporter
import logging
import json
import os
...
class AIJudge(Logger):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.transcriptions_path = os.path.join(EVALUATIONS_DIR, "test_data", "preprocessor", "transcriptions_data.jsonl")
            self.preprocessed_transcriptions_path = os.path.join(EVALUATIONS_DIR, "test_data", "preprocessor", "preprocessings.jsonl")
            self.summary_path = os.path.join(PREPROCESSOR_DIR, "evaluation_results", "judge_evaluation_summary.md")
            self.execution_path = os.path.join(PREPROCESSOR_DIR, "evaluation_databases", "judge_executions.jsonl")
            ...
            self.log(f"AI Judge initialized successfully with model: {self.client.model}")
        ...
        try:
            ...
            for idx, (trans_obj, prep_obj) in enumerate(zip(transcriptions, preprocessed_transcriptions), 1):
                try:
                    ...
                    messages = self.prompt_builder.build(transcription, preprocessed_transcription)
