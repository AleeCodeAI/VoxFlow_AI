...
class Retries(BaseModel):
    ...
    success: bool = Field(description="Whether the transcription was successful after retries")

class TranscriptionReport(BaseModel):
    retries: List[Retries] = Field(description="List of retries for each chunk that failed to transcribe")
    total_chunks: int = Field(description="Total number of chunks that were supposed to be transcribed")
    chunks_completed: int = Field(description="Total number of chunks that were successfully transcribed")
    total_time_ms: Optional[int] = Field(default=None, description="Total pipeline execution time in milliseconds")