...
class PreprocessorEvaluationResult(BaseModel):
    ...
    chunk_completeness: bool = Field(
        description="Whether all chunks were processed completely"
    )
    llm_retries: int = Field(
        description="Number of retries made by the LLM during processing"
    )
    output_existence: bool = Field(
        description="Indicates if the output file exists after processing"
    )
    session_integrity: bool = Field(
        description="Indicates if the session data remains intact after processing"
    )
