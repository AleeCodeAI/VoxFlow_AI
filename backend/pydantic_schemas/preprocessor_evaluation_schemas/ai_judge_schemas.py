...
class Result(BaseModel):
    ...
    meaning_preservation: str = Field(
        description="Score for meaning preservation: HIGH | MODERATE | LOW"
    )
    information_loss: str = Field(
        description="The amount of information lost during preprocessing: HIGH | MODERATE | LOW"
    )
    preprocessing_quality: str = Field(
        description="How well the preprocessing was done: GOLDEN | ACCEPTABLE | POOR"
    )
    hallucination: str = Field(
        description="How much AI hallucinated while preprocessing: HIGH | MODERATE | LOW"
    )
